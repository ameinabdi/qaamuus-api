...
export default class BookService {
  ...
  async create(data) {
    ...
    try {
      const record = await BookRepository.create(data, {
        ...this.options,
        transaction,
      });
  ...
  async update(id, data) {
    ...
    try {
      const record = await BookRepository.update(
        id,
        data,
        {
          ...this.options,
   ...
   */
  async findById(id) {
    return BookRepository.findById(id, this.options);
  }
  ...
  async findAllAutocomplete(search, limit) {
    return BookRepository.findAllAutocomplete(
      search,
      limit,
      this.options,
    );
  ...
  async findAndCountAll(args) {
    return BookRepository.findAndCountAll(
      args,
      this.options,
    );
  }
