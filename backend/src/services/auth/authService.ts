...
const accountSid = 'AC60cc7145edc4cf0514f896deffb6e58e';
const authToken = 'f973f3d6b17a32f9cd0e9e501a095230';
...
class AuthService {
  ...
  static async signupPhone(
    ...
    const verificationNumber = randomize('0', 4);

    try {
      const existingUser = await UserRepository.findByTelephone(
        phoneNumber,
        email,
        options,
      );
      ...
      const newUser = await UserRepository.createFromAuthPhone(
        {
          fullname,
          phoneNumber,
          email,
      ...
      client.messages
        .create({
          body: `Salam ${newUser.fullName} Waakan Furaha Xaqiijinta Xisaabtaada /n Fiiro: Waxa Loogu Talagalay Hal Mar. /n furaha: ${ verificationNumber }`,
          from: '+17573308535',
          to: `${newUser.phoneNumber}`
        }).then(message => console.log('allssssssssss', message));
  ...
  static async signinStudent(
    ...
    try {
      const user = await UserRepository.findByTelephone(
        phoneNumber,
        phoneNumber,
        options,
      );
   ...
   * @param {*} email
   */
  static async sendPasswordResetPhone(
    language,
    phoneNumber,
    options,
  ) {
    ...
    try {
      ...
      const userData = await UserRepository.generatePasswordResetPhone(
        phoneNumber,
        verificationNumber,
        options,
      );
      client.messages
        .create({
          body: `Salam ${userData.fullName} Waakan Furaha Xaqiijinta Xisaabtaada /n Fiiro: Waxa Loogu Talagalay Hal Mar. /n furaha: ${ verificationNumber }`,
          from: '+17573308535',
  ...
  static async sendPasswordResetEmail(
   ...
   /**
   * Sends a Sms To the phone.
   *
   * @param {*} language
   * @param {*} email
   */
  static async sendPhoneSms(
    phone,
    options,
  ) {
     
    const verificationCode = randomize('0000');
    try {

      const newUser = await UserRepository.generatePhonelVerificationCode(
        phone,
        verificationCode,
        options,
      );
      client.messages
      .create({
        body: `Salam ${newUser.fullName} Waakan Furaha Xaqiijinta Xisaabtaada /n Fiiro: Waxa Loogu Talagalay Hal Mar. /n furaha: ${ verificationCode }`,
        from: '+17573308535',
        to: `${newUser.phoneNumber}`
      }).then(message => console.log('allssssssssss', message));
      const token = jwt.sign(
   ...
   * @param {*} options
   */
  static async verifyPhone(code, options) {
    const currentUser = options.currentUser;

    const user = await UserRepository.findByPhoneVerificationCode(
      code,
      options,
    );
    ...
    if (!user) {
      throw new Error400(
        options.language,
        'auth.phoneAddressVerificationEmail.invalidToken',
      );
    }
    ...
    if (
      ...
      throw new Error400(
        options.language,
        'auth.phoneAddressVerificationEmail.signedInAsWrongUser',
        user.phoneNumber,
        currentUser.phoneNumber,
      );
    }

     UserRepository.markPhoneVerified(
      user.id,
      options,
    );
    const token = jwt.sign(
   ...
   * @param {*} options
   */
  static async passwordResetPhone(
    phoneNumber,
    code,
    password,
    options: any = {},
