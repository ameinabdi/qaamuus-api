...
class SubscriptionRepository {
  ...
  static async create(data, options: IRepositoryOptions) {
    ...
    const record = await options.database.subscription.create(
      {
        ...lodash.pick(data, [
          'amount',
          'paymentType',
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    let record = await options.database.subscription.findByPk(
      id,
      {
        transaction,
      },
  ...
  static async findById(id, options: IRepositoryOptions) {
    ...
    const record = await options.database.subscription.findByPk(
      id,
      {
        include,
        transaction,
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return options.database.subscription.count(
      {
        where: {
          ...filter,
          tenantId: tenant.id,
  ...
  static async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.telephoneRange) {
        const [start, end] = filter.telephoneRange;
    ...
    let {
      rows,
      count,
    } = await options.database.subscription.findAndCountAll({
      where,
      include,
  ...
  static async findAllAutocomplete(query, limit, options: IRepositoryOptions) {
    ...
    const records = await options.database.subscription.findAll(
      {
  ...
  /**
   * Fills an array of Subscription with relations and files.
   *
   * @param {Array} rows
   * @param {Object} [options]
   */
